...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  function z = zlevs3(h,zeta,theta_s,theta_b,hc,N,type,scoord)
%
...
%                 'new2008' (Sasha, 2008)
%                 'kau2015' refine surface stretching for special cases
...
elseif (scoord=='kau2015')
    disp('--- using s-coord Kau(2015)')
    Gcoord = 0.16 + abs(sc).^0.8.*(1+sc).^0.2 + (0.3*exp(-1./(max(abs(sc), eps))))./(max(abs(sc), eps).^1.7);
    cff=hc*sc.*Gcoord;
    cff1=Cs;
  for k=1:N
