...
%
% FIX 1 (this version): along_dimname now actually reads dir_map.(dname).dim
% ('eta_rho' or 'xi_rho'), instead of rebuilding a placeholder '<dir>_pts'
% name that partit doesn't recognize as partitionable.
%
% FIX 2 (this version, IMPORTANT -- verify before running): `dating` MUST
% NOT extend past the actual last date covered by your flux-computation
% script (dbrt_900_imporve.m's date_end, e.g. "20221125"). If `dating`
% is set elsewhere in your workspace/session, make sure its end date
% matches -- do not just trim it here without checking that upstream
% value, since extending coverage requires rerunning the flux computation
% itself, not just this writer.
...
if ~exist(flux_out_path,'dir'); mkdir(flux_out_path); end
    ...
    % REQUIRED along-boundary dimension name -- must be exactly 'eta_rho' or
    % 'xi_rho' for partit to recognize and decompose it correctly, matching
    % the same convention zeta_west/zeta_south etc already use in bry.nc}
    dir_map = struct( ...
        'west',  struct('varname','up_west', 'src', west_Fx,  'dim','eta_rho'), ...
        'east',  struct('varname','up_east', 'src', east_Fx,  'dim','eta_rho'), ...
        'south', struct('varname','vp_south','src', south_Fy, 'dim','xi_rho'), ...
        'north', struct('varname','vp_north','src', north_Fy, 'dim','xi_rho') ...
    );
    ...
    for d = 1:length(dating)
        ...
        for j = 1:length(dirstr)
            ...
            along_dimname = dir_map.(dname).dim;    % 'eta_rho' or 'xi_rho' -- FIXED
